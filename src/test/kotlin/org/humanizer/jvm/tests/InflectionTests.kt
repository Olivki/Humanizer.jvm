...
class PluralizeTests : BehaviorSpec({
    val data = listOf(
        "search" to "searches",
        "switch" to "switches",
        "fix" to "fixes",
    ...
    Given("a list of words in singular form") {
        forAll(data) { (input, expected) ->
            When("calling pluralize on \"$input\"") {
                Then("it should become \"$expected\"") {
                    input.pluralize() shouldBe expected
                }
            }
        }
    }
...
class SingularizeTests : BehaviorSpec({
    val data = listOf(
        "search" to "searches",
        "switch" to "switches",
        "fix" to "fixes",
    ...
    Given("a list of words in singular form") {
        forAll(data) { (expected, input) ->
            When("calling singularize on \"$input\"") {
                Then("it should become \"$expected\"") {
                    input.singularize() shouldBe expected
                }
            }
        }
    }
...
class NoDoublesTest : BehaviorSpec({
    val data = listOf(
        "search" to "searches",
        "switch" to "switches",
        "fix" to "fixes",
    ...
    Given("a list of with pairs of words in singularForm:pluralForm") {
        When("grouping by plural") {
            val doubles = data.asSequence().groupBy { it.first }.map { (key) ->
                val count = data.count { it.first == key }
                Pair(key, count)
            }.filter { it.second > 1 }.joinToString(",") { it.first }
    
            Then("it should be empty for all groups") {
                doubles.shouldBeEmpty() // doubles shouldBe ""
            }
        }
    }
})